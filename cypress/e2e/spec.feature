Feature: Powerus
  ...
  Scenario Outline: <testCaseTitle> - should give 3 salary differences
    ...
    When User clicks on Kostenlos Gehalt checken
    When User has "<educationtype>" education
    When User has "<yearsOfExp>" experience
    When User travels in a "<travelType>" manner
    When User lives in "<state>"
    When User clicks on Kostenlos Dein Gehalt sehen to view result
  ...
  Scenario: TC#3 In apprenticeship, cannot travel,2-5 year exp should get 2 salary differences
    ...
    When User selects Elektriker Gehalt Übersicht
    When User clicks on Kostenlos Gehalt checken
    When User has "Aktuell in der Ausbildung" education
    When User has "2-5 Jahre" experience
    When User travels in a "Keine Reisen" manner
    When User lives in "Berlin"
    When User clicks on Kostenlos Dein Gehalt sehen to view result
  ...
  Scenario: TC#4  Finished Ausbildung, cannot travel,5-10 year exp should get only one salary difference
    ...
    When User selects Elektriker Gehalt Übersicht
    When User clicks on Kostenlos Gehalt checken
    When User has "Abgeschlossene Berufsausbildung" education
    When User has "5-10 Jahre" experience
    When User travels in a "Keine Reisen" manner
    When User lives in "Brandenburg"
    When User clicks on Kostenlos Dein Gehalt sehen to view result
  ...
  Scenario: TC#5  no education, can travel,20+ year exp should get 0 salary differences
    ...
    When User selects Elektriker Gehalt Übersicht
    When User clicks on Kostenlos Gehalt checken
    When User has "Nein / Noch nicht anerkannt" education
    When User has "20+ Jahre" experience
    When User travels in a "Häufig" manner
    When User lives in "Bremen"
    When User clicks on Kostenlos Dein Gehalt sehen to view result
