class QuestionnairePage {
  elements = {
    checkSalary: () => cy.get('app-salary-checker-cta').contains('Kostenlos Gehalt checken'),
    educationType: (titleText: string) => cy.get(`app-hoverable-card[title='${titleText}']`),
    yearsOfExperience: (experience: string) => cy.get('div.chip-text').contains(experience),
    travelType: (titleText: string) => cy.get(`app-hoverable-card[title='${titleText}']`),
    StateDropdown: () => cy.get('mat-select#mat-select-0'),
    StateSelection: (optionText: string) => cy.get('span.mat-option-text').contains(optionText),
